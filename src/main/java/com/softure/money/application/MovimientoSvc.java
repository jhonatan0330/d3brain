...
public class MovimientoSvc extends BasicSvc<MovimientoDTO, MovimientoFilterDTO> {
	...
	public MovimientoDTO consultaXId(String llave) throws ServerException {
		if(llave==null) throw new ServerException("La llave del DTO se encuentra vacia. Movimiento");
...
//		movimiento.setSaldoFinal(dto.getSaldoFinal());
	...
	public MovimientoDTO inactivar(MovimientoDTO dto, String token) throws ServerException {
		...
		TurnoDTO turnoMovimiento = null;
		boolean crearNuevoRegistro = false;
		
		if(movimiento.getTurno()!=null){
			turnoMovimiento = turnoService.consultaXId(movimiento.getTurno());
		...
		if(crearNuevoRegistro){ 
			...
			contra.setCuenta(movimiento.getCuenta());
			contra.setMonto(movimiento.getMonto());
			if(movimiento.getTipo().compareTo(MovimientoDTO.ENTRADA_INGRESO)==0){
				contra.setTipo(MovimientoDTO.SALIDA_GASTO);
			}else{
				contra.setTipo(MovimientoDTO.ENTRADA_INGRESO);
			}
			contra = guardar(contra, token);
		}else{
			...
			if(cuenta.getFechaConciliacion()!=null)
				if(cuenta.getFechaConciliacion().compareTo(movimiento.getFechaEvento())>=0) throw new ServerException("No puede registrar un movimiento con fecha inferior a la fecha de conciliacion");
			if(anterior!=null){
			...
			//if(dto.getSaldo().compareTo(dto.getSobregiro().negate())<0) throw new ServerException("La cuenta sobrepasa el limite de sobregiro. Saldo: "+cuenta.getSaldo() + ". Sobregiro:" + cuenta.getSobregiro() + ". Cuenta:" + cuenta.getNombre());
		}
	...
	@Override
	public MovimientoDTO consultaUnica(MovimientoFilterDTO dto) throws ServerException {
		return super.consultaUnica(dto);
	}
	
	@Override
	public int contarResultados(MovimientoFilterDTO dto) throws ServerException {
		return super.contarResultados(dto);
	}
	
	@Override
	public List<MovimientoDTO> listarConsulta(MovimientoFilterDTO dto)
	...
	public MovimientoDTO guardar(MovimientoDTO dto, String token) throws ServerException {
		...
		//if(tipo==null) throw new ServerException("El catalogo "+ dto.getCategoria() + " no existe");
		//if(tipo.getEstado().compareTo(ConstantesGenerales.ESTADO_ACTIVO)!=0) throw new ServerException("El rubro de "+ tipo.getNombre() + " no se encuentra activo");
		//if(dto.getTipo()==null){
		...
		/*if(dto.getTipo().compareTo(MovimientoDTO.TRANSFERENCIA)==0){
			//Debo crear otro movimiento para la cuenta destino con los valores contrarios
			MovimientoDTO contra = new MovimientoDTO();
			...
			contra.setMonto(dto.getMonto());
			contra.setMontoAplicado(dto.getMonto());
			contra.setTipo(MovimientoDTO.ENTRADA_INGRESO);
			contra.setTurno(dto.getTurno());
			contra.setRelacionado(dto.getLlaveTabla());
			...
			contra = super.guardar(contra);
			contra = reorganizar(contra, cuentaDestino.getSobregiro());
			recalcular(contra, cuentaDestino.getSobregiro());
		}*/
		return dto;
		// END Movimiento_guardar
	}

// BEGIN region aditionalMethods
	private MovimientoDTO recalcular(MovimientoDTO inicial, BigDecimal valorMinimoCuenta, String token) throws ServerException {
		...
		while (inicial.getSiguiente()!=null) {
			...
			if(modificado){
				if(siguiente.getSaldoFinal().compareTo(valorMinimoCuenta.negate())<0) throw new ServerException("La cuenta sobrepasa el limite de sobregiro. Saldo: "+siguiente.getSaldoFinal() + ". Sobregiro:" + valorMinimoCuenta +  ". Fecha:" + siguiente.getFechaEvento()+ ". Cuenta:" + inicial.getCuenta());
				siguiente = super.actualizar(siguiente, token);
			}
			inicial = siguiente;
		}
	...
	private MovimientoDTO reorganizar(MovimientoDTO inicial, BigDecimal valorMinimoCuenta, String token) throws ServerException {
		...
		if(inicial.getSaldoFinal().compareTo(valorMinimoCuenta.negate())<0) throw new ServerException("La cuenta sobrepasa el limite de sobregiro. Saldo: "+ SoftureUtil.formatMoney(inicial.getSaldoFinal()) + ". Sobregiro:" + SoftureUtil.formatMoney(valorMinimoCuenta)+ ". Fecha:" + inicial.getFechaEvento()+ ". Cuenta:" + inicial.getCuenta());
		MovimientoDTO siguiente = null;
		if(ultimo!= null){
