...
public class GPSRest {
	...
	private final GPSLocalizacionSvc localizacionGPS;
	private final GPSGetDevicesByQueryService getDevicesByQueryService;
	...
	private final GPSReportLocationsService reportLocationsService;

	public GPSRest(@Lazy GPSLocalizacionSvc localizacionGPS, @Lazy GPSGetDevicesByQueryService getDevicesByQueryService,
			@Lazy GPSGetDevicesByTokenService getDevicesByTokenService,
			@Lazy GPSEnrollDeviceService enrollDeviceService, @Lazy GPSReportLocationsService reportLocationsService) {
		this.localizacionGPS = localizacionGPS;
		this.getDevicesByQueryService = getDevicesByQueryService;
		this.getDevicesByTokenService = getDevicesByTokenService;
		this.enrollDeviceService = enrollDeviceService;
		this.reportLocationsService = reportLocationsService;
	}

	@PostMapping(value = "/enroll-device")
	public SharedIdResponse enroll(@RequestHeader("Authorization") String token, @RequestBody DeviceVO device)
	...
	public List<GPSDispositivoDTO> getdevicesByQuery(@RequestHeader("Authorization") String token)
			throws ServerException {
	...
	public List<GPSDispositivoDTO> getdevicesByQuery(@RequestHeader("Authorization") String token,
			@PathVariable(name = "query", required = false) String pQuery) throws ServerException {
		return getDevicesByQueryService.call(pQuery);
	}

	@PostMapping(value = "/getGPSLocation")
	public List<GPSLocalizacionDTO> getGPSLocation(@RequestHeader("Authorization") String token,
			@RequestBody GPSLocalizacionFilterDTO location) throws ServerException {
		return localizacionGPS.listarFullByDay(location);
	}

	@PostMapping(value = "/save-locations")
	public SharedIdResponse savesLocation(@RequestHeader("Authorization") String token,
			@RequestBody List<GPSLocalizacionDTO> locations) throws ServerException {
