	...
	public OrganizacionDTO obtenerPrincipalPublic(String ipRequest) throws ServerException {
		try {
			...
			result.setPropiedades(configuracionSvc.obtenerPropiedades(PropiedadValorDefinidoDTO.ORGANIZACION,
					result.getLlaveTabla(), null, userPublic, false));
			return result;
		} catch (Exception e) {
...
// BEGIN region aditionalMethods
	...
	/*
	 * public OrganizacionDTO copiar(String org, String token) throws
	 * ServerException{ OrganizacionDTO bd = consultaXId(org); // ASumo que la bd ya
	 * esta creada ProcesoFilterDTO filtroP = new ProcesoFilterDTO();
	 * filtroP.setEstado(ConstantesGenerales.ESTADO_ACTIVO); List<ProcesoDTO>
	 * proceso = procesoService.listarConsulta(filtroP);
	 * DbContextHolder.setDataSourceType(DbType.SLAVE); try { for (ProcesoDTO
	 * procesoDTO : proceso) { procesoService.guardar(procesoDTO, token); }
	 * //necesito } catch (Exception e) { // TODO: handle exception }
	 * DbContextHolder.resetDataSourceType(); return bd; }
	 */
