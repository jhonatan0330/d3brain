...
public class TipoDetallePedido {
	...
	@Autowired private CallDocumentCRUD saveUpdateInactivateDocumentFunction;
	...
	public void cargarConsultaCampo(PedidoVentaCaracteristicaDTO pCampo, String token) throws ServerException{
		if(pCampo.getDocumento()!=null){
			List<PropiedadDTO> tarifario = Propiedades.obtenerVariosParametro(pCampo.getCampoDTO(), Propiedades.DETALLE_TARIFARIO);
			if(tarifario != null && tarifario.isEmpty()) tarifario = null;
			
			if(!pCampo.getModificado())pCampo.setDetalles(detallePedidoVentaService.listarCompleto(pCampo.getDocumento(), tarifario, null, null, token));
			...
			if(!unico.isEmpty() && pCampo.getDetalles()!=null && pCampo.getDetalles().size()!=0) {
				pCampo.setValorText(pCampo.getDetalles().get(0).getProductoCodigo() + " - " + pCampo.getDetalles().get(0).getNombre());
			}
		}
	}
	...
	public void validarPrepararCampo(PedidoVentaCaracteristicaDTO pCampo, String token) throws ServerException{
		...
		if(Propiedades.obtenerParametro(pCampo.getCampoDTO(), Propiedades.PERMISO_CAMPO_OPCIONAL)==null && (pCampo.getDetalles()==null || pCampo.getDetalles().size()==0)) throw new ServerException("Es obligatorio colocar el campo " + pCampo.getCampoDTO().getNombre());
		...
		if(pCampo.getDetalles()!=null && !pCampo.getDetalles().isEmpty()){	
			...
			if(!formula.isEmpty()){
				if(pCampo.getDependientes()==null) throw new ServerException("Revise los dependientes del tipo detalle"); 
			...
			for (DetallePedidoVentaDTO detalle : pCampo.getDetalles()) {
				...
				if(agregar)agrupados.add(detalle);
				texto = texto + "(" +detalle.getCantidad() + ") " + detalle.getNombre() + " ";
			}
			pCampo.setDetalles(agrupados);
			String unico = Propiedades.obtenerValor(pCampo.getCampoDTO(), Propiedades.UNICO_PRODUCTO);
			if(!unico.isEmpty()) {
				if(pCampo.getDetalles()!=null && pCampo.getDetalles().size()!=1) throw new ServerException("Cuando un producto tiene la propiedad unica solo puede venir un producto a guardar");
		...
		if(pCampo.getDocumento()!=null){
			...
			List<DetallePedidoVentaDTO> detallesActuales = detallePedidoVentaService.listarCompleto(pCampo.getDocumento(), tarifario, null, null, token);
			
			for (DetallePedidoVentaDTO detalle : agrupados) {
			...
			if (detallesActuales!=null && detallesActuales!=null && !detallesActuales.isEmpty()){
				if(pCampo.getDetalles()==null) pCampo.setDetalles(new ArrayList<DetallePedidoVentaDTO>());
				for (DetallePedidoVentaDTO detalleEliminado : detallesActuales) {
					...
					pCampo.getDetalles().add(detalleEliminado);
				}
			}
		}else{
			pCampo.setDetalles(agrupados);
			...
			for(DetallePedidoVentaDTO detalle : pCampo.getDetalles()){
				if(detalle.getEstado()==null || detalle.getEstado().compareTo(ConstantesGenerales.ESTADO_ACTIVO)==0){
					...
					if(detalle.getCantidadPromocion()!=null && detalle.getCantidadPromocion() >0){
						if(detalle.getCantidadPromocionBase().compareTo(0)==0) throw new ServerException("El valor base de promocion no puede ser cero");
						long cantPromo = 0;
						cantPromo = Math.round(Math.floor(detalle.getCantidad().intValue() / detalle.getCantidadPromocionBase()));
					...
					/*if(detalle.getValorUnitario().compareTo(detalle.getValorMinimo())<0)
						throw new ServerException("El valor unitario es menor al minimo.");
					if(detalle.getValorUnitario().compareTo(detalle.getValorMaximo())>0)
	...
	private boolean sonCaracteristicasIguales(DetallePedidoVentaDTO detalleAgrupado, DetallePedidoVentaDTO detalle) throws ServerException{
		...
		if(detalleAgrupado.getCaracteristicas()!=null && !detalleAgrupado.getCaracteristicas().isEmpty()){
			if(detalle.getCaracteristicas()==null) return true;
			if(detalle.getCaracteristicas().isEmpty())return true;
			if(detalle.getCaracteristicas().size()!= detalleAgrupado.getCaracteristicas().size())return true;
			
			
			for(PedidoVentaCaracteristicaDTO  dcp: detalle.getCaracteristicas()){
				if(dcp.getCampoDTO()==null) dcp.setCampoDTO(caracteristicaService.consultaXIdProducto(dcp.getCampo()));
				for(PedidoVentaCaracteristicaDTO  dcpa: detalleAgrupado.getCaracteristicas()){
					if(dcpa.getCampoDTO()==null) dcpa.setCampoDTO(caracteristicaService.consultaXIdProducto(dcpa.getCampo()));
					if(dcp.getCampoDTO().getCodigo().compareTo(dcpa.getCampoDTO().getCodigo())==0){
						if(dcp.getValorOpcion()==null){
							...
							if(dcpa.getValorOpcion()!=null && dcp.getValorOpcion().compareTo(dcpa.getValorOpcion())!=0){
								//detalleAgrupado.setLlaveTabla(null);
								return false;
							}
						}
	...
	public PedidoVentaCaracteristicaDTO guardarCampo(PedidoVentaCaracteristicaDTO pCampo, String token) throws ServerException {
		List<DetallePedidoVentaDTO> result = null;
		if(pCampo.getDetalles()!=null && ! pCampo.getDetalles().isEmpty()){
	...
	public PedidoVentaCaracteristicaFilterDTO consultarDatosBase(PedidoVentaCaracteristicaFilterDTO pCampo) throws ServerException {
		...
		if(pTercero!=null) {
			...
			if(tercero==null) throw new ServerException("Debe seleccionar un " + pTercero.getTexto());
		}
		...
		if(funcionProductos==null) {
			if(tercero!=null){
				...
				pBase.setProductos(productoService.listarProductoCampo(pBase.getLlaveTabla(), pCampo.getFiltroParametro()));
			}			
		}else {
			PropiedadDTO pCampoFuncion = Propiedades.obtenerParametro(pCampo.getCampoDTO(), Propiedades.PRODUCTOS_FUNCION_CAMPO);
			...
			if(valorCampo==null) throw new ServerException("Revise la configuracion debe tener un campo relacionado para la funcion" + pCampoFuncion.getTexto());
			pBase.setProductos(productoService.listarProductoFuncion(funcionProductos.getLlaveTabla(), valorCampo, pCampo.getFiltroParametro(), pCampo.getSecurityToken()));
		}
		if(pBase.getProductos()!=null && !pBase.getProductos().isEmpty()){
			...
			pBase.setProductos(detallarProductos2Plantilla(pBase.getProductos(), pCampo.getCampoDTO(), null, tercero, (tarifarioFuncion!=null)?tarifarioFuncion.getLlaveTabla():null, pCampo.getDependientes(), pCampo.getSecurityToken()));
			...
			for (ProductoDTO productoDTO : pBase.getProductos()) {
				...
				if(!existeCategoria){
					...
					categoria.setPropiedades(configuracionSvc.obtenerPropiedades(PropiedadValorDefinidoDTO.PLANTILLA, categoria.getLlaveTabla(), null, configuracionSvc.getUserFlex(pCampo.getSecurityToken())));
	...
	private CategoriaProductoDTO ordenar(List<CategoriaProductoDTO> categorias) throws ServerException{
		if(categorias==null) categorias = new ArrayList<CategoriaProductoDTO>();
		...
		while(categorias.size()>1) {
			...
			if(ultimo.getNodoSuperior() == null) ultimo.setNodoSuperior("NODO1476");
			...
			if(padre == null) {
				...
				if(categoria == null) throw new ServerException("No se encuentra la categoria principal. " + ultimo.getNodoSuperior());
				categorias.add(categoria);
			}else {
				if(padre.getHijos()==null) padre.setHijos(new ArrayList<CategoriaProductoDTO>());
				int j = 0;//Ordenar alfabeticamenta
				while(j <padre.getHijos().size() && padre.getHijos().get(j).getNombre().compareTo(ultimo.getNombre())<0) {
					j++;
				}
	...
	private List<ProductoDTO> detallarProductos2Plantilla(
			List<ProductoDTO> productos, 
			DocumentoPlantillaCaracteristicaDTO pCampo, 
			CategoriaProductoDTO categoria, 
			String tercero, 
			String propiedadFuncionTarifario, 
			List<PedidoVentaCaracteristicaDTO> parametrosFuncionTarifario,
		...
		if(productos!=null && !productos.isEmpty()){
			...
			List<ProductoDTO> productosSimplificados = detallePedidoVentaService.simplificarConsultaBDProductos(productos); 
			for (ProductoDTO productoDTO : productos) {
		        ...
		        productoDTO.setDetallePlantilla(detallePedidoVentaService.consultaCompleta(filtroPlantilla, tarifario, tercero, propiedadFuncionTarifario, parametrosFuncionTarifario, productosSimplificados, token));
		        if(imagenes ==null){
		        	if(productoDTO.getImagen()==null && categoria !=null) productoDTO.setImagen(categoria.getImagen());
		        }else{
		        ...
		        //productoDTO.setPropiedades(configuracionSvc.obtenerPropiedades(PropiedadValorDefinidoDTO.PLANTILLA, productoDTO.getLlaveTabla(), null, pCampo.getSecurityToken()));
			}
		}
		return productos;
	}
	...
	public PedidoVentaCaracteristicaDTO inactivar(PedidoVentaCaracteristicaDTO pCampo, String token) throws ServerException {
		if(pCampo.getDetalles()!=null && ! pCampo.getDetalles().isEmpty()){
			for (DetallePedidoVentaDTO detalle : pCampo.getDetalles()) {
				detalle.setTransaccionInactivo(pCampo.getTransaccionRegistro());
				detallePedidoVentaService.inactivar(detalle, token);
	...
	public void novedadParcial(PedidoVentaCaracteristicaDTO pCampo, String token) throws ServerException {
		...
		if(pCampo!=null){
			...
			if(documentoPrincipal ==null) documentoPrincipal = pedidoService.consultaXId(pCampo.getDependientes().get(0).getValorOpcion());
			PedidoVentaDTO expediente = pedidoService.obtenerCamposCompletos(documentoPrincipal, token);
			List<DetallePedidoVentaDTO> detallesExpediente = detallePedidoVentaService.listar2Documento(expediente.getLlaveTabla());
			
			List<DetallePedidoVentaDTO> detallesFinalExpediente = new ArrayList<DetallePedidoVentaDTO>();//Expediente: Como queda el expediente
			List<DetallePedidoVentaDTO> detallesFinalNuevo = new ArrayList<DetallePedidoVentaDTO>();//Nuevo : Como queda el expediente actual
				...
				for (DetallePedidoVentaDTO detalleExpediente : detallesExpediente) {
					...
					if(detalleDocumento.getProducto().compareTo(detalleExpediente.getProducto())==0){
						if(detalleDocumento.getCantidad().compareTo(detalleExpediente.getCantidad())==0){
							...
							BigDecimal nuevoTotal = detalleExpediente.getCantidad().add(detalleDocumento.getCantidad().negate());
							if(nuevoTotal.compareTo(BigDecimal.ZERO)<0) throw new ServerException("En una devolución no se puede colocar una cantidad mayor a la inicial de " + detalleExpediente.getCantidad());
							//Cuadra los inventarios con promocion
							if(detalleDocumento.getCantidadPromocion().compareTo(0)!=0){
								BigDecimal cantidadRestanteTotales = detalleExpediente.getCantidadTotal().add(detalleDocumento.getCantidadTotal().negate());
								detalleDocumento.setCantidadPromocion(cantidadRestanteTotales.add(nuevoTotal.negate()).intValue());
							...
							detallePedidoVentaService.createFieldsProduct(detalleExpediente);// Aqui porque antes no me actualiza la cantidad en las novedades parciales
							detallesFinalExpediente.add(detalleExpediente);
							detallesExpediente.remove(detalleExpediente);
						}
						break;
			...
			saveUpdateInactivateDocumentFunction.updateWithoutTransaction(expediente, pCampo.getDocumento(), token, true);
