			...
			if(!rango.isEmpty()){
				...
				if(rangoMaximo!=null){
					...
					if(periodos.length>1) {
						Duration lt = Duration.parse("P" + periodos[1]);
						fechaInicial.add(Calendar.SECOND, (int)(lt.getSeconds()));
					}
					if(fechaInicial.before(fechaFinal))throw new ServerException("La fecha "+ pCampo.getCampoDTO().getNombre() + " no debe superar un periodo de " + rangoMaximo.getTexto() );//+ );
				}
				switch (pCampo.getValorAuxiliar()){
					case "D":{pCampo.setValorText(SoftureUtil.formatDate(pCampo.getValorFecha()));break;}
					case "M":{pCampo.setValorText(SoftureUtil.formatDateMonth(pCampo.getValorFecha()));break;}
					case "A":{pCampo.setValorText(SoftureUtil.formatDateYear(pCampo.getValorFecha()));break;}
					case "R":{pCampo.setValorText(DurationFormatUtils.formatDurationWords(pCampo.getValorNumero().longValue(), true, true));break;}
				}
			}else{
				if(Propiedades.obtenerValor(pCampo.getCampoDTO(), Propiedades.FECHA_CON_HORA).isEmpty()){
					...
					if(Propiedades.obtenerValor(pCampo.getCampoDTO(), Propiedades.FECHA_SIN_CALENDAR).isEmpty()){
						...
						if (hora.get(Calendar.HOUR)!=0) {
							...
							pCampo.setValorNumero(pCampo.getValorNumero().add(new BigDecimal(hora.get(Calendar.HOUR_OF_DAY)*60*60*1000)));
						}
						if (hora.get(Calendar.MINUTE)!=0) {
							if (pCampo.getValorText()==null) {
								pCampo.setValorText(hora.get(Calendar.MINUTE) + " Minutos");
							}else {
								pCampo.setValorText(pCampo.getValorText() + "," + hora.get(Calendar.MINUTE) + " Minutos");
							}
