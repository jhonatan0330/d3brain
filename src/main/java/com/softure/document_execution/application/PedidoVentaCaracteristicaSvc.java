...
public class PedidoVentaCaracteristicaSvc
	...
	public PedidoVentaCaracteristicaDTO consultaXId(String llave) throws ServerException {
		if (llave == null)
			throw new ServerException("La llave del DTO se encuentra vacia. PedidoVentaCaracteristica");
	...
	public PedidoVentaCaracteristicaDTO guardar(PedidoVentaCaracteristicaDTO dto, String token) throws ServerException {
		...
		if (dto.getValorText() != null && dto.getValorText().length() > 4000)
			throw new ServerException("Se excedio la cantidad de caracteres del texto. (4000)");
		if (dto.getValorNumero() != null && dto.getValorNumero().compareTo(BigDecimal.ZERO) == 0)
			dto.setValorNumero(null);
		if (dto.getValorNumero() != null && dto.getValorNumero().compareTo(new BigDecimal(9999999999999999.0)) > 0)
			throw new ServerException("Se excedio del numero maximo");
	...
	// buscar
	public List<PedidoVentaCaracteristicaDTO> listar2getMessageMailDestiny(
			List<PedidoVentaCaracteristicaDTO> documentIds, List<RelacionInternaDTO> fieldId) throws ServerException {
		if (documentIds == null || documentIds.isEmpty() || fieldId == null || fieldId.isEmpty())
			return null;
		List<PedidoVentaCaracteristicaDTO> fieldsInternal = pedidoVentaCaracteristicaMapper
				.listar2getMessageMailDestiny(documentIds, fieldId);
		// Existia un error al consultar una lista larga de relaciones para un mensaje
		// Esa lista en el segundo nivel de un campo multiple se bloqueaba porque
		// validaba que el campoDTO no fuera nulo
	...
	public PedidoVentaCaracteristicaDTO buscarActivo(PedidoVentaCaracteristicaDTO dto, Integer historico)
			throws ServerException {
		if (dto == null || dto.getDocumento() == null || dto.getCampo() == null)
			throw new ServerException("Error al consultar el campo previo por falta de datos");
		...
		if (Propiedades.isFunctionNotFreeMarker(propFunction.getValor())) {
			try {
				...
				if (dependientes != null) {
					// En formularios de pedidos de bbx se envia a calclualr el flete sin
					// dependientes
			...
			if (iDependiente.getValorOpcion() == null) {
				if (iDependiente.getExpedientes() != null) {
					...
					for (PedidoVentaDTO iExpediente : iDependiente.getExpedientes()) {
						...
						if (iExpediente.getEstado() == null
								|| iExpediente.getEstado().compareTo(SharedConstants.STATE_INACTIVE) != 0) {
		...
		if (Propiedades.isFunctionNotFreeMarker(sqlFuncionDecision.getValor())) {
			try {
				...
				return pedidoVentaCaracteristicaMapper.calcularFechaFuncion(
						SoftureUtil.formatFunction(sqlFuncionDecision.getLlaveTabla()), documento, token,
						dependientesOrdenados);
			} catch (Exception e) {
	...
	// Aqui traigo en el valor auxiliar traigo la plantila y en el estado el estado
	// del documento
	...
	public List<PedidoVentaCaracteristicaDTO> camposOcupadosCroquis(String sqlFuncionDecision, String campoId,
		...
		try {
			return pedidoVentaCaracteristicaMapper.consultarCamposOcupados(
					SoftureUtil.formatFunction(sqlFuncionDecision), campoId, token, dependientes);
		} catch (Exception e) {
			throw new ServerException(e.getMessage(), "Funcion de campos ocupados : " + sqlFuncionDecision);
		}
	}

	public PedidoVentaCaracteristicaDTO getDependent(String field, List<PedidoVentaCaracteristicaDTO> dependents)
			throws ServerException {
		if (field == null)
			throw new ServerException("No se identifica el campo dependiente");
		if (dependents == null)
			return null;
	...
	public void validarDependientes(DocumentoPlantillaCaracteristicaDTO campo,
			List<PedidoVentaCaracteristicaDTO> dependientes) throws ServerException {
		List<PropiedadDTO> codigoDepende = Propiedades.obtenerVariosParametro(campo, Propiedades.DEPENDENT_PROPS);
		if (codigoDepende == null || codigoDepende.isEmpty())
			return;
		// En algunso casos de relacionar me apsaba que llaamba el mismo dependiente y
		// no venia
		if (codigoDepende.size() == 1 && codigoDepende.get(0).getCampo().compareTo(campo.getLlaveTabla()) == 0)
			return;
		...
		// if(dependientes.size()!=codigoDepende.size()) throw new ServerException("El
		// numero de dependientes no concuerda. Tipo Expediente" +
		// codigoDepende.size());
	...
	public List<PedidoVentaCaracteristicaDTO> ordenarAlfabeticaDepende(List<PedidoVentaCaracteristicaDTO> dependientes)
		...
		if (dependientes == null)
			throw new ServerException("Por favor cierra y vuelve a cargar la pagina");
		for (int i = 0; i < dependientes.size(); i++) {
			PedidoVentaCaracteristicaDTO iDepende = dependientes.get(i);
			// Esto fue por un error en ccarga al consultar el producto pero no se si pasa
			// lo mismo en otros sistemas
			if (iDepende.getCampoDTO() == null)
				iDepende.setCampoDTO(campoDocumentoService.consultaXId(iDepende.getCampo()));
			if (i == 0) {
				...
				int j = 0;
				while (j < dependentOrderList.size() && dependentOrderList.get(j).getCampoDTO().getCodigo()
						.compareTo(iDepende.getCampoDTO().getCodigo()) <= 0) {
					j++;
				}
		...
		for (int i = dependents.size() - 1; i >= 0; i--) {
			for (int j = i - 1; j >= 0; j--) {
				if (dependents.get(i).getCampoDTO().getCodigo()
						.compareTo(dependents.get(j).getCampoDTO().getCodigo()) == 0) {
					dependents.remove(i);
					break;
				}
			}
	...
	public String validarUnique(PedidoVentaCaracteristicaDTO value) throws ServerException {
		if (value == null)
			throw new ServerException("Trae los datos del campo");
		return pedidoVentaCaracteristicaMapper.getUnique(value);
	}

	public PedidoVentaCaracteristicaDTO consultarSQLCampoGenerarDocumento(String sqlFuncionDecision, String documentoId,
			String modificadorId, String iteratorId) throws ServerException {
		try {
			return pedidoVentaCaracteristicaMapper.consultarSQLCampoGenerarDocumento(
					SoftureUtil.formatFunction(sqlFuncionDecision), documentoId, modificadorId, iteratorId);
		} catch (Exception e) {
			throw new ServerException(e.getMessage());
		}
	}
	...
	/*
	 * Creado porq un cliente desea ver en al lista unos campos que no hacen parte
	 * de la plantilla asi que con una funcion los traemos
	 */
	public List<PedidoVentaCaracteristicaDTO> camposEspecialesPlantilla(String sqlFuncion, String documentoId)
			throws ServerException {
		try {
			return pedidoVentaCaracteristicaMapper.camposEspecialesPlantilla(SoftureUtil.formatFunction(sqlFuncion),
					documentoId);
		} catch (Exception e) {
