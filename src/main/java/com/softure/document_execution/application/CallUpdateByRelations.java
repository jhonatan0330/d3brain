...
public class CallUpdateByRelations {
	...
	public void call(CallDocumentCRUD pCrud, PedidoVentaDTO pDTO, String pToken) throws ServerException {
		...
		if(_fieldWithProperties.isEmpty()) return;
	...
	private void relacionExternaDocumentos(PedidoVentaCaracteristicaDTO pCampo, String token, Map<String, PedidoVentaDTO> pDocumentsToUpdate) throws ServerException {
			...
			for (PedidoVentaCaracteristicaDTO dependiente : pCampo.getDependientes()) {
				if (dependiente.getCampo().compareTo(propiedadDTO.getValor()) == 0) {
					...
					for (RelacionInternaDTO iRelacion : relaciones) {
						...
						PedidoVentaCaracteristicaDTO campoDestino = pedidoVentaCaracteristicaService.consultaUnica(campoDestinoFilter);
						...
						if (campoDestino != null) {
							...
							campoDestino.setCampoDTO(documentoPlantillaCaracteristicaService.consultaXId(campoDestino.getCampo()));
							...
							campoDestino.setCampoDTO(documentoPlantillaCaracteristicaService.cargarComplementos(campoDestino.getCampoDTO(), token));
							...
							for (PedidoVentaDTO iDocumentoRelacionar : pCampo.getExpedientes()) {
								if (propiedadDTO.getKey().compareTo(Propiedades.RELACIONAR_DOCUMENTOS) == 0) {
									PedidoVentaDTO _same_document = (pCampo.getCampo().equals(propiedadDTO.getValor()))?pCampo.getPrincipal():iDocumentoRelacionar;
									campoDestino.getExpedientes().add(_same_document);
									/*if(campoValor.isEmpty()) relacionExpedienteService.relacionarExpedienteDocumento(campoDestino.getLlaveTabla(), _same_document.getLlaveTabla(), token,
											campoDestino.getCampoDTO().getNombre(), (_same_document.getDinero()==null)?null:_same_document.getDinero().getSaldo(), 
							...
							DocumentoPlantillaCaracteristicaDTO _field = documentoPlantillaCaracteristicaService.consultaXId(iRelacion.getCampo());
