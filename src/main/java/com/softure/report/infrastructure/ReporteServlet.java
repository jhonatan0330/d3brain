	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			...
			if (resultado != null && resultado.getContent() != null) {
				...
				// downloadFile(response, input, name +"_" +
				// SoftureUtil.formatDateMassiveFile(new Date()).replaceAll("\\\\", "_") + "." +
				// tipoReporte.toLowerCase());
				downloadFile(response, resultado.getContent(),
						name + "_" + SoftureUtil.formatDateMassiveFile(new Date()).replaceAll("\\\\", "_") + "."
								+ tipoReporte.toLowerCase());
			}
		} catch (Exception e) {
