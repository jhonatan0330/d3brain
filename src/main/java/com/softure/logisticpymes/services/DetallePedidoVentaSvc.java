...
import com.softure.logisticpymes.domain.dto.DetalleCaracteristicaProductoDTO;
...
import com.softure.logisticpymes.domain.filter.DetalleCaracteristicaProductoFilterDTO;
...
import com.softure.logisticpymes.infrastructure.mybatis.mapper.DetallePedidoVentaMapper;
...
public class DetallePedidoVentaSvc extends BasicSvc<DetallePedidoVentaDTO, DetallePedidoVentaFilterDTO> {
	...
	public DetallePedidoVentaDTO consultaXId(String llave) throws ServerException {
		if(llave==null) throw new ServerException("La llave del DTO se encuentra vacia. DetallePedidoVenta");
	...
	public DetallePedidoVentaDTO inactivar(DetallePedidoVentaDTO dto, String token) throws ServerException {
		...
		if(dto.getTransaccionInactivo()==null)throw new ServerException("Ingrese la transaccion de inactivar");
	...
	@Override
	public List<DetallePedidoVentaDTO> listarConsulta(DetallePedidoVentaFilterDTO dto)
...
// BEGIN region aditionalMethods
	public List<DetallePedidoVentaDTO> listarCompleto(
			String documento, 
			List<PropiedadDTO> tarifario, 
			String propiedadFuncion,
		...
		if(base  ==null || base.isEmpty()) return new ArrayList<DetallePedidoVentaDTO>();
		...
		for (DetallePedidoVentaDTO detallePedidoVentaDTO : base) {
			result.add(consultaCompleta(detallePedidoVentaDTO, tarifario, null, propiedadFuncion, parametrosFuncionTarifario, productosSimplificados, token));
		}
		return result;
	}
	...
	private void formatValidarCampo(PedidoVentaCaracteristicaDTO dto, DetallePedidoVentaDTO campo) throws ServerException {
		switch(dto.getCampoDTO().getFormato()){
			case DocumentoPlantillaCaracteristicaDTO.FECHA:
				if(dto.getValorFecha()!=null){
					...
					if(Propiedades.obtenerParametro(dto.getCampoDTO(), Propiedades.PERMISO_CAMPO_OPCIONAL)==null) throw new ServerException("Falta la opcion " + dto.getCampoDTO().getNombre() + " del producto "+ campo.getNombre() + ". Cant: " + campo.getCantidad());
				}
				break;
			...
			case DocumentoPlantillaCaracteristicaDTO.PROCESO:
				if(dto.getValorOpcion()!=null){
					...
					if(documento ==null) throw new ServerException("La opcion escogida no esta correctamente configurada." + dto.getCampo());
			...
			case DocumentoPlantillaCaracteristicaDTO.TEXTO:
				if(dto.getValorText()!=null)dto.setValorText(dto.getValorText().toUpperCase());	
				break;
		}
	}
	...
	public List<ProductoDTO> simplificarConsultaBDProductos(List<ProductoDTO> productos)throws ServerException{
		...
		if(bases!=null && bases.size()!=0) {
			...
			List<ProductoCaracteristicaDTO> camposBases = productoCaracteristicaService.listarProductoSimplificar(bases);
			for (ProductoDTO iProductoDTO : bases) {
				...
				if(propiedadesBases!=null && !propiedadesBases.isEmpty()) {
					for (PropiedadDTO propiedadDTO : propiedadesBases) {
						if(propiedadDTO.getCampo().compareTo(iProductoDTO.getLlaveTabla())==0) iProductoDTO.getPropiedades().add(propiedadDTO);
					}
				}
				...
				if(camposBases!=null && !camposBases.isEmpty()) {
					for (ProductoCaracteristicaDTO campoDTO : camposBases) {
						if(campoDTO.getBase().compareTo(iProductoDTO.getLlaveTabla())==0) iProductoDTO.getCampos().add(campoDTO);
					}
				}
			}
		}
		List<PropiedadDTO> propiedadesProducto = configuracionSvc.listarProductoSimplificar(productos);
		List<ProductoCaracteristicaDTO> camposProducto = productoCaracteristicaService.listarProductoSimplificar(productos);
		for (ProductoDTO productoDTO : result) {
			...
			if(propiedadesProducto!=null && !propiedadesProducto.isEmpty()) {
				for (PropiedadDTO propiedadDTO : propiedadesProducto) {
					if(propiedadDTO.getCampo().compareTo(productoDTO.getLlaveTabla())==0) productoDTO.getPropiedades().add(propiedadDTO);
				}
			}
			...
			if(camposProducto!=null && !camposProducto.isEmpty()) {
				for (ProductoCaracteristicaDTO campoDTO : camposProducto) {
					if(campoDTO.getBase().compareTo(productoDTO.getLlaveTabla())==0) productoDTO.getCampos().add(campoDTO);
				}
			}
	...
	public DetallePedidoVentaDTO consultaCompleta(
			DetallePedidoVentaDTO dto, 
			List<PropiedadDTO> tarifario, 
			String tercero, 
			String propiedadFuncionTarifario, 
			List<PedidoVentaCaracteristicaDTO> parametrosFuncionTarifario, 
			List<ProductoDTO> productosSimplificados, 
			String token)throws ServerException{
		DetallePedidoVentaDTO result;
		if (dto.getProducto() == null)throw new ServerException("Para esta operacion se debe colocar el producto");
		...
		if (producto == null)throw new ServerException("Para esta operacion se debe colocar el producto con identificador valido");
		...
		if(propiedadFuncionTarifario !=null) {
			result.setTarifas( tarifaService.obtenerTarifaFuncion(propiedadFuncionTarifario, producto, parametrosFuncionTarifario) );
		}else {
			//Consulto las tarifas
			if(tarifario!= null){
				result.setTarifas(consultarTarifas( tarifario, tercero,dto.getProducto()));
				if(producto.getProductoBase()!=null && (result.getTarifas()==null || result.getTarifas().isEmpty())) result.setTarifas(consultarTarifas(tarifario, tercero, producto.getProductoBase()));
			}			
		}
		...
		if (result.getCantidad() == null) result.setCantidad(BigDecimal.ZERO);
		if (result.getValorUnitario() == null) result.setValorUnitario(BigDecimal.ZERO);
		...
		if (caracteristicas != null && !caracteristicas.isEmpty()) {
			...
			for (ProductoCaracteristicaDTO productoCaracteristicaDTO : caracteristicas) {
				...
				nuevaCaracteristica.setOrden(productoCaracteristicaDTO.getOrden() + 4 );//Por los cuatro parametros inciales
				...
				//Propiedades.obtenerParametro(dto.getCampoDTO(), Propiedades.PERMISO_CAMPO_OBLIGATORIO)!=null
				
				List<PropiedadDTO> parametros = configuracionSvc.obtenerPropiedades(PropiedadValorDefinidoDTO.CAMPO, productoCaracteristicaDTO.getLlaveTabla(), null, getUserFlex(token));
				
				parametros.add(Propiedades.crearParametro(PropiedadValorDefinidoDTO.CAMPO, null, Propiedades.PERMISO_CAMPO_MODIFICABLE, Propiedades.TRUE, null));
				...
				if(productoCaracteristicaDTO.getFormato().compareTo(DocumentoPlantillaCaracteristicaDTO.NUMERO)==0) {
					if(Propiedades.obtenerParametro(nuevaCaracteristica, Propiedades.NUMERO_FUNCION_SQL)!=null)
						nuevaCaracteristica.getPropiedades().add(Propiedades.crearParametro(PropiedadValorDefinidoDTO.CAMPO, null, Propiedades.DEPENDE, "***PRODUCTO", null));
					if(Propiedades.obtenerParametro(nuevaCaracteristica, Propiedades.NUMERO_FORMULA)!=null)
						nuevaCaracteristica.getPropiedades().add(Propiedades.crearParametro(PropiedadValorDefinidoDTO.CAMPO, null, Propiedades.DEPENDE, "***TOTAL", null));
				}
				
				if(nueva==null)	nueva = new PedidoVentaCaracteristicaDTO();
	...
	public void createFieldsProduct(DetallePedidoVentaDTO field) throws ServerException {
		if(field.getCaracteristicas()== null) field.setCaracteristicas(new ArrayList<PedidoVentaCaracteristicaDTO>());
		if(field.getPropiedades()==null) field.setPropiedades(new ArrayList<PropiedadDTO>());
		...
		if(field.getTarifas()!=null && !field.getTarifas().isEmpty()) {
			...
			PropiedadDTO pCampoUnitario = Propiedades.obtenerParametro(field, Propiedades.PRODUCTO_CAMPO_VALOR_UNITARIO);
			if(pCampoUnitario==null || pCampoUnitario.getValor()==null || pCampoUnitario.getValor().compareTo("***UNIDAD")==0) {//Sucede que como se copian las propiedades entonces de crean aveces esta propiedad pero el campo no
				...
				//baseValorUnitario.getPropiedades().add(Propiedades.crearParametro(PropiedadValorDefinidoDTO.CAMPO, null, Propiedades.PERMISO_CAMPO_MODIFICABLE, Propiedades.TRUE, null));
				baseValorUnitario.getPropiedades().add(Propiedades.crearParametro(PropiedadValorDefinidoDTO.CAMPO, null, Propiedades.NUMERO_MONEDA, Propiedades.TRUE, null));
				...
				field.getPropiedades().add(Propiedades.crearParametro(PropiedadValorDefinidoDTO.CAMPO, null, Propiedades.PRODUCTO_CAMPO_VALOR_UNITARIO, baseValorUnitario.getLlaveTabla(), null));
			}else {
			...
			if(baseValorUnitario==null) throw new ServerException("Se ha presentado un problema consultando el campo de valor unitario" + field.getNombre());
		}
		...
		if(pCampoCantidad==null|| pCampoCantidad.getValor()==null || pCampoCantidad.getValor().compareTo("***CANTIDAD")==0) {//Sucede que como se copian las propiedades entonces de crean aveces esta propiedad pero el campo no
			//creo un campo con el valor unitario
			...
			baseCantidad.getPropiedades().add(Propiedades.crearParametro(PropiedadValorDefinidoDTO.CAMPO, null, Propiedades.PERMISO_CAMPO_MODIFICABLE, Propiedades.TRUE, null));
		...
		if(baseCantidad==null) throw new ServerException("Se ha presentado un problema consultando el campo de valor unitario" + field.getNombre());
	...
	private BigDecimal getTarifaInitial(DetallePedidoVentaDTO field) {
		if(field.getTarifas()==null || field.getTarifas().isEmpty()) return BigDecimal.ZERO;
		return field.getTarifas().get(0).getValor();
	}

	private List<TarifaDTO> consultarTarifas(List<PropiedadDTO> tarifario, String tercero, String producto) throws ServerException {
	...
	public DocumentoPlantillaCaracteristicaDTO crearCampoBaseProducto() {
		...
		baseProducto.getPropiedades().add(Propiedades.crearParametro(PropiedadValorDefinidoDTO.CAMPO, null, Propiedades.PERMISO_CAMPO_BLOQUEAR, Propiedades.TRUE, null));//NO se puede modificar
		return baseProducto;
	}
