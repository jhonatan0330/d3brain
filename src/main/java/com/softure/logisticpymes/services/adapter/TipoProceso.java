...
import com.softure.logisticpymes.services.refactor.CallDocumentListBySQLFunction;
import com.softure.logisticpymes.services.refactor.CallDocumentListFromFieldProcess;
import com.softure.logisticpymes.services.refactor.CallDocumentUpdateFromAutomatic;
...
public class TipoProceso {
	...
	@Autowired private CallDocumentUpdateFromAutomatic updateDocumentFunction;
			...
			if(Propiedades.obtenerParametro(pCampo.getCampoDTO(), Propiedades.AUTOLOAD_SAVE)!=null) {
				...
				if(!multiple.isEmpty()){
					...
					pCampo.setExpedientes(
							listDocumentBySQLFunction.execute(pCampo.getCampoDTO(), pCampo.getCampoDTO(), pCampo.getDependientes(), null, funcionConsulta, null, token)
							);
	...
	private void validarMultiple(PedidoVentaCaracteristicaDTO pCampo, String token) throws ServerException{
		...
		PedidoVentaCaracteristicaFilterDTO calculado = CallDocumentCommons.calcularValoresTotalesCampo(toFilter(pCampo, token), campoValor, relacionExpedienteService);
	...
	public PedidoVentaCaracteristicaFilterDTO consultarDatosBase(PedidoVentaCaracteristicaFilterDTO pCampo) throws ServerException {
		return listDocumentFromFieldProcessFunction.execute(pCampo, caracteristicaService.consultaUnicaConComplementos(pCampo.getCampo(), pCampo.getSecurityToken()));
	}
