...
public class CallManageTransition {
	...
	@Autowired private ProcesoTransicionMapper procesoTransicionMapper;
	
	public ProcesoTransicionDTO execute(
		...
		if(dto.getPropiedades()==null) dto.setPropiedades(propiedadService.obtenerPropiedades(PropiedadValorDefinidoDTO.TRANSICION, dto.getLlaveTabla(), null, userID));
		propiedadService.validarFuncionConsultandoPropiedad(dto, PropiedadValorDefinidoDTO.TRANSICION, expediente, documentoDTO.getLlaveTabla(), userID);
		...
		case ProcesoEstadoDTO.TIPO_DECISION:
			...
			respuesta = execute(respuesta, expediente, documentoDTO, valorModificador, afectado, relacionAnterior, tokenSystem.getLlaveTabla(), transaccion);			
			break;
		case ProcesoEstadoDTO.TIPO_ITERADOR:
			respuesta = getNextTransition(dto.getEstadoLLegada(), null);
			respuesta = execute(respuesta, expediente, documentoDTO, valorModificador, afectado, relacionAnterior, token, transaccion);//Por si siguen decisiones
		...
		case ProcesoEstadoDTO.TIPO_API:
			...
			break;
		default:
			UsuarioDTO responsable = assignResponsibleToActivity(expediente,filtroEstado.getLlaveTabla(), filtroEstado.getNombre(), documentoDTO.getLlaveTabla(), token);
	...
	private ProcesoTransicionDTO executeAPI(String estadoLlegada, PedidoVentaDTO expedienteDTO, PedidoVentaDTO documentoDTO, String token) throws ServerException {
		...
		apiDTO.setPropiedades( propiedadService.obtenerPropiedadesSinEntidad(PropiedadValorDefinidoDTO.ESTADO, estadoLlegada, null , getUserId(token)) );
	...
	private ProcesoTransicionDTO resolveStateDesition(String decision, String llaveTablaDocumento, String llaveModificador, String token) throws ServerException {
		...
		PropiedadDTO propiedadFuncion = propiedadService.obtenerPropiedad(PropiedadValorDefinidoDTO.ESTADO, decision, Propiedades.DECISION_SQL, getUserId(token));
	...
	public UsuarioDTO assignResponsibleToActivity(String pedido, String estado, String estadoNombre, String modificador,String token) throws ServerException {//, DocumentoPlantillaDTO plantilla
		if(estado==null) return null;
		ActividadDTO responsable = new ActividadDTO();
		String userID = getUserId(token);
		PropiedadDTO propiedadFuncion = propiedadService.obtenerPropiedad(PropiedadValorDefinidoDTO.ESTADO, estado, Propiedades.FUNCION_SQL_ESTADO_ASIGNAR, userID); 
		if(propiedadFuncion !=null){
			...
			propiedadFuncion = propiedadService.obtenerPropiedad(PropiedadValorDefinidoDTO.ESTADO, estado, Propiedades.ESTADO_ASIGNAR, userID);
			if(propiedadFuncion !=null){
	...
	private PedidoVentaDineroDTO moveBalanceDocument(String expediente, String securityToken, ProcesoTransicionDTO transicion, 
	...
	public ProcesoTransicionDTO gestionarTransicionReversa(
		...
		PedidoVentaDineroDTO nuevoValor = moveBalanceDocument(expediente, token, dto, valorModificador, null);//aqui es nulo porque ya existe
		//Creo la relacion del documento Gestor
		...
		assignResponsibleToActivity(expediente,filtroEstado.getLlaveTabla(), filtroEstado.getNombre(), documento.getLlaveTabla(), token);
