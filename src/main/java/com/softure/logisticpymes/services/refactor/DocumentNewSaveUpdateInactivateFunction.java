...
public class DocumentNewSaveUpdateInactivateFunction {
	...
	private PedidoVentaDTO saveAfterIdentifyTransaction(PedidoVentaDTO dto, String token) throws ServerException {
		...
		if(!api.isEmpty()) apiService.execute(api, dto, null, token);
		dto.setCaracteristicas(null);//Por error al serializar
		return pedido;
	...
	private void manageState(PedidoVentaDTO pedido, String plantillaNombre, String token, String transaccion) throws ServerException{
	...
	private void validateBalance(PedidoVentaDTO pedido, DocumentoPlantillaDTO plantilla) throws ServerException {
		...
		if(!total.isEmpty() ){
			PedidoVentaCaracteristicaDTO campoValor= DocumentCommonsFunction.obtenerValor(pedido.getCaracteristicas(), total);
	...
	private void validateFields(PedidoVentaDTO dto, DocumentoPlantillaDTO plantilla, String token) throws ServerException {
					...
					for (PedidoVentaCaracteristicaDTO iCampoDocumento : dto.getCaracteristicas()) {
						if(iCampoDocumento.getModificado()) {
							if(Propiedades.obtenerParametro(iCampoDocumento.getCampoDTO(), Propiedades.PERMISO_CAMPO_MODIFICABLE)==null) {
								...
								mensajeError = mensajeError + usuarioService.consultaXId(getUserID(token)).getNombre() + " no tiene permisos de modificar ese campo";
	...
	private List<PedidoVentaCaracteristicaDTO> saveInternalFields(PedidoVentaDTO dto, String token) throws ServerException{
	...
	private void validateDates(PedidoVentaDTO pedido, String caracteristicaFecha) throws ServerException {
		if(caracteristicaFecha.isEmpty()){
			...
			PedidoVentaCaracteristicaDTO campoFecha= DocumentCommonsFunction.obtenerValor(pedido.getCaracteristicas(), caracteristicaFecha);
	...
	public void manageTemplateTypes(PedidoVentaDTO dto, DocumentoPlantillaDTO plantilla, String token) throws ServerException {
		//Viene de inactivar
		if(plantilla==null) {
	...
	public void saveRole(PedidoVentaDTO dto, String token) throws ServerException {
		...
		if(dto.getEstado()==null || dto.getEstado().compareTo(ConstantesGenerales.ESTADO_ACTIVO)==0){
			...
			String campoConsecutivo = propiedadService.obtenerUnica(PropiedadValorDefinidoDTO.PLANTILLA, dto.getPlantilla(), Propiedades.CONSECUTIVO, getUserID(token));
