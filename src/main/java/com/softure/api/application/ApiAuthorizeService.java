...
public class ApiAuthorizeService {
	...
	public ApiAuthorizeService(@Lazy OrganizacionSvc organizationService) {
	...
	public void call(String apiKey, String token) throws ServerException {
		...
		if (apiKeyOrganization == null)
			throw new ServerException(
					"Notifica al administrador que no se encuentra configurada la clave de acceso x-api-key en la organizacion");
		if (apiKey.compareTo(apiKeyOrganization) != 0)
			throw new ServerException(
					"El x-api-key que envias no concuerda con el registrado en la plataforma: " + apiKey);
		return;
	}
	...
	private void getApiKeyHeader(String user) throws ServerException {
		...
		if (apiKeyOrganization != null && apiKeyOrganization.isEmpty())
