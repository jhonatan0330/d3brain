...
// BEGIN region aditionalMethods
	...
	public UsuarioRolDTO consultaValidandoCaracteristicas(UsuarioRolDTO dto)throws ServerException{
		...
		if(rol.getEstado().compareTo(SharedConstants.STATE_ACTIVE)!=0)throw new ServerException("Rol inactivo.\n" + rol.getNombre());
		
		UsuarioDTO usuario = usuarioService.consultaXId(dto.getUsuario());
		if(usuario==null)throw new ServerException("No se identifico usuario");
		if(usuario.getEstado().compareTo(SharedConstants.STATE_ACTIVE)!=0)throw new ServerException("Usuario inactivo.\n" + usuario.getNombre());
