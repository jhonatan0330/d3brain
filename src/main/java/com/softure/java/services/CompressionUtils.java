...
public class CompressionUtils {
	...
	private static BufferedImage rotateImage(BufferedImage buffImage, double angle) {
		...
		return rotatedImage;
		/*
		 * AffineTransform transform = new AffineTransform(); transform.rotate(radian,
		 * buffImage.getWidth() / 2, buffImage.getHeight() / 2); AffineTransformOp op =
		 * new AffineTransformOp(transform, AffineTransformOp.TYPE_BILINEAR); return
		 * op.filter(buffImage, null);
		 */
	}
	...
	// Esto lo usaba en UploadSvc por ftp, pero deno cuadrarlo para que sirva con
	// una propiedad
	...
	public static byte[] compress(byte[] data) throws IOException {
		...
		// ImageWriter writer =
		// ImageIO.getImageWritersByFormatName(imageType.getExtension()).next();
		...
		/*
		 * File input = new File("digital_image_processing.jpg"); BufferedImage image =
		 * ImageIO.read(input);
		 * 
		 * File compressedImageFile = new File("compress.jpg"); OutputStream os =new
		 ...
		 * 
		 * param.setCompressionMode(ImageWriteParam.MODE_EXPLICIT);
		 * param.setCompressionQuality(0.5f); writer.write(null, new IIOImage(image,
		 * null, null), param);
		 * 
