...
import d3.accounting.application.base.ResultMapService;
import d3.accounting.application.base.TimeFrameService;
...
public class VoucherCalculateService {
	...
	private final VoucherGetService voucherService;
	private final ResultMapExtendService mapService;
	private final TimeFrameService timeFrameService;
	private final ResultMapService resultMapService;

	public VoucherCalculateService(@Lazy PlanGetAccountService accountService, @Lazy VoucherGetService voucherService,
			@Lazy ResultMapExtendService mapService, @Lazy TimeFrameService timeFrameService,
			@Lazy ResultMapService resultMapService) {
	...
	public void call(String voucherId, String action) throws ServerException {
		...
		for (VoucherLine item : _voucher.getRecords()) {
			...
			saveMap(item.getLine().getAccount(), item.getLine().getFactDate(), item.getLine().getPositive(),
					item.getLine().getNegative(), item.getLine().getValue(), acumulador, action);
			if (item.getReferences() != null && !item.getReferences().isEmpty()) {
				for (AccountRecordAuxiliarDTO iAux : item.getReferences()) {
					if (iAux.getAccount() != null)
						saveMap(iAux.getAccount(), item.getLine().getFactDate(), item.getLine().getPositive(),
								item.getLine().getNegative(), item.getLine().getValue(), acumulador, action);
				}
			}
		}
		...
		for (ResultMapDTO cpv : acumulador.values()) {
			...
			if (timeFrame != null)
				mapService.updateBalance(cpv.getAccount(), timeFrame.getStartDate(), timeFrame.getLevel(),
						cpv.getValueInProcessing());
		}
	}

	private void saveMap(String accountId, Date factDate, BigDecimal positive, BigDecimal negative, BigDecimal value,
			Map<String, ResultMapDTO> acumulador, String action) throws ServerException {
		...
		for (ResultMapDTO resultMapDTO : mapItems) {
			...
			if (account.getOperation().compareTo(AccountConst.OPERATION_MINUS) == 0)
				_valueToApply = _valueToApply.negate();
			...
			if (account.getType().compareTo(AccountConst.TYPE_AUXILIAR) == 0) {
				if (accountParent != null && accountParent.getParent() != null) {
					...
					if (accountParentTop != null && accountParentTop.getParent() != null) {
						AccountDTO _accountGroupParent = accountService.findAccountByDocumentId(account.getCatalog(),
								account.getDocument(), accountParentTop.getKey());
