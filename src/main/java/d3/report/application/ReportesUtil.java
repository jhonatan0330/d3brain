...
public class ReportesUtil {
	...
	public static byte[] exportarReporteExcel(String reportejrxml, Map<String, Object> parametrosReporte,
			Connection conexion, JasperReportCache pCache, String pReportKey) throws Exception {
		try {
			if (parametrosReporte.containsKey(REPORTE_ENCABEZADO_EXCEL)) {
				JasperReport encabezado = pCache.getReport(parametrosReporte.get(REPORTE_ENCABEZADO_EXCEL).toString(),
						pReportKey);
				// JasperReport encabezado = JasperCompileManager.compileReport(new
				// ByteArrayInputStream(parametrosReporte.get(REPORTE_ENCABEZADO_EXCEL).toString().getBytes("utf-8")));
			...
			for (Map.Entry<String, Object> entry : parametrosReporte.entrySet()) {
				if (entry.getKey().startsWith(P_SUBREPORT)) {
					...
					// JasperReport jasperSubReport = JasperCompileManager.compileReport(new
					// ByteArrayInputStream(entry.getValue().toString().getBytes("utf-8")));
			...
			// ByteArrayInputStream(reportejrxml.getBytes("utf-8")));
			...
			throw new ServerException("OOM ERROR XLS EXPORT: " + oom.getMessage()
					+ ". Este reporte esta degradando el sistema el administrador esta siendo informado por favor consulta menos datos.");
		} catch (Exception e) {
	...
	private static String replaceReport(String report, String nameSR, String nameParameter) {
		report = report.replace("<queryString", "<parameter name=\"" + nameParameter
				+ "\" class=\"net.sf.jasperreports.engine.JasperReport\"/>\r\n	<queryString");
		report = report.replace("<subreportExpression><![CDATA[\"" + nameSR + ".jasper\"]]></subreportExpression>",
				"<subreportExpression class=\"net.sf.jasperreports.engine.JasperReport\" ><![CDATA[$P{" + nameParameter
						+ "}]]></subreportExpression>");
		return report;
	}
	...
	private static String replaceHeader(String report, String nameSubreport) throws Exception {
		...
		report = report.replace("<queryString", "<parameter name=\"NOMBRE\" class=\"java.lang.String\"/>\r\n"
				...
				+ nameSubreport + "\" class=\"net.sf.jasperreports.engine.JasperReport\"/>\r\n " + "   <queryString");
		int inicio = report.indexOf("<pageHeader>");
		if (inicio < 0)
			throw new Exception("No se encuentra el inicio del tag <pageHeader> ");
		int fin = report.substring(inicio + 1).indexOf("</pageHeader>");
		if (fin < 0)
			throw new Exception("No se encuentra el fin del tag </pageHeader> ");
	...
	private static String replaceFooter(String report) throws Exception {
		...
		if (inicio < 0)
			throw new Exception("No se encuentra el inicio del tag <pageFooter> ");
		int fin = report.substring(inicio + 1).indexOf("</pageFooter>");
		if (fin < 0)
			throw new Exception("No se encuentra el fin del tag </pageFooter> ");
	...
	public static byte[] exportarReportePDF(String reportejrxml, Map<String, Object> parametrosReporte,
			Connection conexion, JasperReportCache pCache, String pReportKey) throws Exception {
		ByteArrayOutputStream vByteOutputStream = new ByteArrayOutputStream();
		try {
			if (parametrosReporte.containsKey(REPORTE_ENCABEZADO)) {
				JasperReport encabezado = pCache.getReport(parametrosReporte.get(REPORTE_ENCABEZADO).toString(),
						pReportKey);
				// JasperReport encabezado = JasperCompileManager.compileReport(new
				// ByteArrayInputStream(parametrosReporte.get(REPORTE_ENCABEZADO).toString().getBytes("utf-8")));
			...
			if (parametrosReporte.containsKey(REPORTE_PIE_PAGINA)) {
				JasperReport piePage = pCache.getReport(parametrosReporte.get(REPORTE_PIE_PAGINA).toString(),
						pReportKey);
				// JasperReport piePage = JasperCompileManager.compileReport(new
				// ByteArrayInputStream(parametrosReporte.get(REPORTE_PIE_PAGINA).toString().getBytes("utf-8")));
			...
			if (jasperPrint.getPages().size() != 0) {
				...
				if (parametrosReporte.get("P_PRINTER_NAME") != null) {
					configuration.setPdfJavaScript(
							"var params = this.getPrintParams();params.interactive=params.constants.interactionLevel.silent;params.pageHandling=params.constants.handling.none;params.printerName='"
									+ parametrosReporte.get("P_PRINTER_NAME") + "';this.print(params);");
				} else {
			...
			throw new ServerException("OOM ERROR PDF EXPORT: " + oom.getMessage()
					+ ". Este reporte esta degradando el sistema el administrador esta siendo informado por favor consulta menos datos.");
		} catch (Exception e) {
			throw new Exception(e.getMessage());
		}
	...
	public static byte[] exportarReporteHTML(String reportejrxml, Map<String, Object> parametrosReporte,
			Connection conexion, JasperReportCache pCache, String pReportKey) throws Exception {
		try {
			...
			throw new ServerException("OOM ERROR HTML EXPORT: " + oom.getMessage()
					+ ". Este reporte esta degradando el sistema el administrador esta siendo informado por favor consulta menos datos.");
		} catch (Exception e) {
			throw new Exception(e.getMessage());
		}
