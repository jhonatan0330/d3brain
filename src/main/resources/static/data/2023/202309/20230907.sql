COMMENT ON TABLE usuario_usrp IS '2023-09-07';
...
and p.proname like ''propiedad_%'') LOOP
		...
		query := query || '' RETURNS void LANGUAGE plpgsql AS '' || chr(36) ||chr(36) || doc_record.definicion || chr(36) || chr(36) ||'' ;'';
		EXECUTE  query;
	END LOOP;
  RETURN;
END;
...
and proargnames IN (''{parametros}'')
and p.proname like ''propiedad_%'') LOOP
		query := ''DROP function if EXISTS '' || doc_record.function_name || '' (character varying[]);'';
		EXECUTE  query;
	END LOOP;
  RETURN;
END;
