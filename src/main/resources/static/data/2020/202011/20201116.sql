COMMENT ON TABLE usuario_usrp IS '2020-11-16';
...
and (proargnames =''{documento}'' or proargnames is null)
and prorettype = 1043
...
and prorettype = 1043
and p.proname like ''propiedad_%'') LOOP
		query := ''DROP function if EXISTS '' || doc_record.function_name || '' (varchar);'';
		EXECUTE  query;
	END LOOP;
  RETURN;
END;
